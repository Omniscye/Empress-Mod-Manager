...
impl ManagedGame {
    ...
    pub fn create_desktop_shortcut(&self) -> Result<()> {
        ...
        let shortcut_path =
            desktop_path.join(format!("Empress - {} - {}.lnk", self.game.name, profile.name));

        #[cfg(target_os = "linux")]
        let shortcut_path =
            desktop_path.join(format!("empress-{}-{}.desktop", self.game.name, profile.name));
