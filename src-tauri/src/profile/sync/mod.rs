use std::{env, fmt::Display, io::Cursor, sync::LazyLock};
...
const API_URL_ENV: &str = "EMPRESS_SYNC_URL";
static API_URL: LazyLock<String> = LazyLock::new(|| {
    env::var(API_URL_ENV).unwrap_or_else(|_| "https://sync.empress.invalid/api".to_string())
});

async fn request(method: Method, path: impl Display, app: &AppHandle) -> reqwest::RequestBuilder {
    let url = format!("{}{path}", API_URL.as_str());
