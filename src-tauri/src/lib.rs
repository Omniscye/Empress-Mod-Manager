...
fn setup(app: &mut App) -> Result<(), Box<dyn std::error::Error>> {
    info!(
        "empress mod manager v{} running on {}",
    ...
    if let Err(err) = state::setup(app.handle()) {
        ...
        app.dialog()
            .message(format!("Failed to launch Empress Mod Manager: {err:?}"))
            .blocking_show();
    ...
    if let Err(err) = app.deep_link().register("empress") {
        warn!("failed to register empress deep link protocol: {:#}", err);
    }
