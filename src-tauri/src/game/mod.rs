...
const GITHUB_API_URL_ENV: &str = "EMPRESS_GAMES_COMMIT_API_URL";
const GAMES_JSON_URL_ENV: &str = "EMPRESS_GAMES_JSON_URL";

const BUNDLED_GAMES_JSON: &str = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/games.json"));

const BUILD_TIME: &str = env!("BUILD_TIME");

static GITHUB_API_URL: LazyLock<Option<String>> =
    LazyLock::new(|| env::var(GITHUB_API_URL_ENV).ok());
static GAMES_JSON_URL: LazyLock<Option<String>> =
    LazyLock::new(|| env::var(GAMES_JSON_URL_ENV).ok());
...
pub async fn update_list_task(app: &AppHandle) -> Result<()> {
    ...
    let Some(url) = GAMES_JSON_URL.as_deref() else {
        info!(
            "skipping games list update because {} is not configured",
            GAMES_JSON_URL_ENV
        );
        return Ok(());
    };
    ...
    info!("updated games list from configured source, version date {date}");
...
async fn get_last_commit_date(app: &AppHandle) -> Result<DateTime<Utc>> {
    ...
    let Some(url) = GITHUB_API_URL.as_deref() else {
        info!(
            "{} is not configured; using the fetch time for the games cache",
            GITHUB_API_URL_ENV
        );
        return Ok(Utc::now());
    };
