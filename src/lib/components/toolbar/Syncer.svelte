...
<Dialog bind:open={mainDialogOpen} title={m.syncer_title()}>
	...
	{#if !brand.syncEnabled}
		...
		<p class="text-primary-300 mt-3 text-sm">
			Local profiles, imports, exports, and the new Ops Center all work normally.
		</p>
	{:else if syncInfo}
	...
	<div class="text-primary-300 mt-4 flex items-center gap-1">
		{#if !brand.syncEnabled}
			<div class="text-primary-400 text-sm">
				Sync controls unlock after an Empress backend is configured.
			</div>
		{:else if auth.user === null}
			<Button
				onclick={onLoginClicked}
	...
	{#if brand.syncEnabled}
		...
		>
			...
			<a target="_blank" href={brand.syncDocsUrl}>{m.syncer_content_help()}</a>
		</div>
	{/if}
</Dialog>
